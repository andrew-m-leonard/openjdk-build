...
if [ ! -d "$(eval echo "\$$BOOT_JDK_VARIABLE")" ]; then
  ...
  if [ ! -x "$bootDir/bin/javac" ]; then
    ...
    if [ -x "/usr/lib/jvm/jdk-${JDK_BOOT_VERSION}/bin/javac" ]; then
      ...
      echo "Downloading GA release of boot JDK version ${JDK_BOOT_VERSION} from https://ci.adoptopenjdk.net/job/build-scripts/job/jobs/job/jdk/job/jdk-linux-x64-hotspot/526/artifact/workspace/target/OpenJDK-jdk_x64_linux_hotspot_2021-11-03-14-05.tar.gz" 
      # make-adopt-build-farm.sh has 'set -e'. We need to disable that for
      # the fallback mechanism, as downloading of the GA binary might fail.
      set +e
      curl -L "https://ci.adoptopenjdk.net/job/build-scripts/job/jobs/job/jdk/job/jdk-linux-x64-hotspot/526/artifact/workspace/target/OpenJDK-jdk_x64_linux_hotspot_2021-11-03-14-05.tar.gz" | tar xpzf - --strip-components=1 -C "$bootDir"
      retVal=$?
      set -e
      if [ $retVal -ne 0 ]; then
