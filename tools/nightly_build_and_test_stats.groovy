  ...
  stage("getTestStats") {
    ...
    buildNamesJson.each { build ->
      ...
      if (build._id.buildName.contains("-pipeline")) {
        ...
        // or upstream project "build-scripts/weekly-openjdkNN-pipeline" started in the last 6 days, as those are weekend weekly release jobs
            ...
            if (!foundNightly) {
              ...
              if (job.status != null && job.status.equals("Done") && job.startBy != null) {
                if (job.startBy.startsWith("timer")) {
                  ...
                  def build_time = new Date(job.timestamp)
                  def now = new Date()
